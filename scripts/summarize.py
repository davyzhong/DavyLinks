...
from config_loader import get_llm_provider_configs, _read_yaml
...
def get_all_llm_configs():
    ...
    if providers:
        return _build_llm_configs(providers)
...
def _build_llm_configs(providers):
    """按优先级排序构建 LLM 配置列表"""
...
def summarize_parallel(articles, llm_configs):
    ...
    def _try_with_fallback(idx, article):
        ...
        while current not in tried:
            ...
            try:
                result = summarize_one(article, config)
        ...
        # 所有 LLM 都失败 — 返回新 dict，不修改原始 article
        return idx, {**article, "summary": "(所有LLM摘要均失败)", "tags": [], "quality_rating": 2, "summarized_by": "none"}, "none"
    ...
    with ThreadPoolExecutor(max_workers=min(len(articles), n_llm)) as pool:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                results[idx] = {**articles[idx], "summary": "(摘要超时)", "tags": [], "quality_rating": 2, "summarized_by": "timeout"}
...
def final_sort(articles):
    ...
    """
    scored = [
        {**a, "final_score": a.get("relevance_score", 0) + a.get("quality_rating", 3) * 2 + a.get("cross_source_score", 0)}
        for a in articles
    ]
...
def main():
    ...
    if not llm_configs:
        ...
        articles = [
            {**a, "summary": "(未生成摘要)", "tags": [], "quality_rating": 3}
            for a in articles
        ]
    else:
        max_summarize = 5
        ...
        # 保留位置映射：None 条目用 fallback 填充，不丢弃
        articles = [
            s if s is not None else {**a, "summary": "(摘要超时)", "tags": [], "quality_rating": 2}
            for s, a in zip(summarized, to_summarize)
        ] + [
            {**a, "summary": "(摘要已跳过)", "tags": [], "quality_rating": 2}
            for a in skip
        ]
